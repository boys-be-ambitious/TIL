...
## 2. 스크래핑 함수
def get_rh_rent_data(loc, ym):
	key = "U01n1ERAP3rooP6xztJBrjzTM95nrzASy%2Bw%2BHZRXCmH1ZYON516m6alV%2F%2BUKNfUtvREtal7DVAX0ZU5ltW2xrg%3D%3D" #인증키
	url = "http://openapi.molit.go.kr:8081/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcRHRent?LAWD_CD=" + str(loc) + "&DEAL_YMD=" + str(ym) + "&serviceKey=" + key
	resp = urlopen(url)
	xml = resp.read().decode('UTF-8')	 # 데이터를 가져오고 UTF-8 String으로 변환	xml = etree.fromstring(xml)
	...
	rh_rent = pd.DataFrame(result, columns=['시군구동', '번지', '본번', '부번', '건물명', '전월세구분', '전용면적', '계약년월', '계약일', '보증금', '월세', '층', '건축년도'])
	return(rh_rent)


## 3. 다운로드 함수 ... 하나의 년월 값만 입력 가능
def download_rent_rh(year_month, num_retries=10, guit=None):
	...
	try:
		for i in range(len(LAWD_CD)):
			...
			if guit:
				...
				print(msg)
			get_data = get_rh_rent_data(LAWD_CD[i], year_month)
