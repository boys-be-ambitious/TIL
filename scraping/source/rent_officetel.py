...
def get_officetel_rent_data(loc, ym):
	key = "U01n1ERAP3rooP6xztJBrjzTM95nrzASy%2Bw%2BHZRXCmH1ZYON516m6alV%2F%2BUKNfUtvREtal7DVAX0ZU5ltW2xrg%3D%3D" #인증키
	url = "http://openapi.molit.go.kr/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcOffiRent?LAWD_CD="+str(loc)+"&DEAL_YMD="+str(ym)+"&serviceKey="+key
	resp = urlopen(url)
	...
	for child in root.iter("item"):
		deposit = child.findtext('보증금')
		rent = child.findtext('월세')
		...
		danji = child.findtext('단지').strip()
		year_of_construction = child.findtext('건축년도')
#		 si_gun_gu = child.findtext('시군구')
	...
	officetel_rent = pd.DataFrame(result, columns=['시군구동', '번지', '본번', '부번', '단지명', '전월세구분', '전용면적', '계약년월', '계약일', '보증금', '월세', '층', '건축년도'])
	return(officetel_rent)

## 3. 다운로드 함수 ... 하나의 년월 값만 입력 가능
def download_rent_officetel(year_month, num_retries=10, guit=None):
	...
	try:
		for i in range(len(LAWD_CD)):
			...
			if guit:
				...
				print(msg)
			get_data = get_officetel_rent_data(LAWD_CD[i], year_month)
