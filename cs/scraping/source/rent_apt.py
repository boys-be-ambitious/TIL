...
## 2. 스크래핑 함수
def get_apt_rent_data(loc, ym):
	key = "U01n1ERAP3rooP6xztJBrjzTM95nrzASy%2Bw%2BHZRXCmH1ZYON516m6alV%2F%2BUKNfUtvREtal7DVAX0ZU5ltW2xrg%3D%3D" #인증키
	resp = urlopen("http://openapi.molit.go.kr:8081/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcAptRent?LAWD_CD="+str(loc)+"&DEAL_YMD="+str(ym)+"&serviceKey="+key)
	...
	for child in root.iter("item"):
		...
		rent = child.findtext('월세금액').strip()
	...
	apt_rent = pd.DataFrame(result, columns=['시군구동', '번지', '본번', '부번', '단지명', '전월세구분', '전용면적', '계약년월', '계약일', '보증금', '월세', '층', '건축년도'])
	return(apt_rent)


## 3. 다운로드 함수 ... 하나의 년월 값만 입력 가능
def download_rent_apt(year_month, num_retries=10, guit=None):
	...
	try:
		for i in range(len(LAWD_CD)):
			...
			if guit:
				...
				print(msg)
			get_data = get_apt_rent_data(LAWD_CD[i], year_month)
