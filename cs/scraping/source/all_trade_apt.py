...
## 2. 스크래핑 함수
def get_apt_trade_data(loc, ym):
	key = "U01n1ERAP3rooP6xztJBrjzTM95nrzASy%2Bw%2BHZRXCmH1ZYON516m6alV%2F%2BUKNfUtvREtal7DVAX0ZU5ltW2xrg%3D%3D" #인증키
	url = "http://openapi.molit.go.kr:8081/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcAptTrade?LAWD_CD=" + str(loc) + "&DEAL_YMD=" + str(ym) +"&serviceKey=" + key
	resp = urlopen(url)
	...
	apt = pd.DataFrame(result, columns=['시군구동', '번지', '본번', '부번',  '단지명', '전용면적', '계약년월', '계약일', '거래금액', '층', '건축년도'])
	return(apt)


## 3. 다운로드 함수 ... 하나의 년월 값만 입력 가능
def download_trade_apt(year_month, num_retries=10, guit=None):
	...
	try:
		for i in range(len(LAWD_CD)):
			...
			if guit:
				...
				print(msg)
			get_data = get_apt_trade_data(LAWD_CD[i], year_month)
...
# for year in ym_list[1:-1]:
params = int(input("params : "))
for year in ym_list[params]:
	ym = year
	print(str(datetime.now())[:16])
	print("year_month : ", ym)

	res = download_trade_apt(ym)
	res.to_excel('../output/apt_trade({0}_{1}).xlsx'.format(ym, today), encoding='utf-8')
	print('='*80)
