...
- 20여명 규모의 팀
- 6명+의 개발자로 구성된 기술팀
- 10명+의 데이터 분석가, 데이터 사이언티스트

- 집중하는 분야 (1시간 42분)
    - 사고리스크를 예측하여 정확한 손익 예측 및 보험료 설계 
        -  1~2% 비용 절감 (2000억 비즈니스, )
    - 최적의 차량 가격을 연구하고 설정
        - 
    - 데이터에 기반한 차량 수요 예측, 차량 배치 최적화
        - 관리가 잘 안 되고 있었음. 
        - 경험과 직관에 의한 판단
            - 대학가: 경차?, 주택가: SUV? -> Nope!
            - 데이터 들여다 보면 다 틀림
            - 데이터에 기반해서 의사결정
    - 차량 리뷰, 유저 평판 데이터를 활용하여 퀄리티 관리, 비용 컨트롤
        - 예를 들면, 차 퀄리티를 높이겠다고 한 달에 4번... 세차 비용 1.5 * 4 = 6만, 연 72만원, 72억으로 1년에 세차비를 씀.
        - 세차 바로 다음 만족도, 두 번째에는 만족도가 낮아짐... 외부 세차 횟수를 줄이고... 내부 세차 횟수를 더 늘리면 돈도 절약하고 퀄리티를 더 높일 수 있다. 이렇게 하면 몇십 억 늘릴 수 있다. 해결해야할 문제가 정말 많다.
    - 타다 데이터 사이언스 문제 해결 (with VCNC 데이터팀)
        - 타다 연 매출 1,000억 대 비즈니스. 데이터 가지고 할 수 있는 문제가 많다.
    - Many More !

---
## 1.13. 데이터 엔지니어로 커리어를 시작하려면?
- Skill Set에 대해 엄청 중요하게 생각하지 않는다.
- 중요하게 생각하는 건 ... **불확실한 데이터에서 확신을 가질 수 있도록 확률과 통계 및 데이터에 대한 지식을 가지신 분**
- 어설프게 익힌 지식은 헤매이게 된다.
- 이론적인 걸 갖고 있는 사람이 확고하게 잘 가져간다.
- 데이터 엔지니어, 인프라 엔지니어... 
